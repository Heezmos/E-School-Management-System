...
export default function ChangePasswordPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      ...
      if (!recovery && data.user.user_metadata?.must_change_password !== true) {
        router.replace("/");
        return;
      }
      setChecking(false);
    });
  }, [router, supabase, recovery]);
  ...
  async function updatePassword(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await fetch("/api/security-events", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: recovery ? "password_recovered" : "password_changed" }) });
    } catch {}
    ...
    <main className="auth">
      <section className="auth-brand">
        <div style={{maxWidth:580}}>
          <div style={{fontSize:13,fontWeight:800,letterSpacing:".12em",textTransform:"uppercase",color:"#b9c8dc",marginBottom:16}}>{recovery ? "Account recovery" : "First login security"}</div>
          <h1>E-School</h1>
          <p>{recovery ? "Create a new password for your E-School account." : "Your temporary password has worked. Before entering your school workspace, create a private password known only to you."}</p>
        </div>
      </section>
      ...
      <section className="auth-form-wrap">
        <form className="auth-card" onSubmit={updatePassword}>
          ...
          <p>{recovery ? "Choose a new password you will use for future E-School sign-ins." : "This password will replace the temporary credential you received."}</p>
