...
export default function SchoolAdminManager({ schoolId, admins }: { schoolId:string; admins:AdminRow[] }) {
  ...
  return <section className="panel tenant-admins" style={{marginTop:18}}>
    <div className="section-heading"><div><span className="eyebrow">ACCESS CONTROL</span><h2>School Administrators</h2><p>Manage every administrator attached to this school tenant.</p></div><span className="count-badge">{admins.length}</span></div>
    ...
    <div className="tablewrap"><table className="premium-table"><thead><tr><th>ADMINISTRATOR</th><th>EMAIL</th><th>FIRST LOGIN</th><th>STATUS</th><th>ACTIONS</th></tr></thead><tbody>
      {admins.length?admins.map(a=><tr key={a.user_id}><td><b>{`${a.first_name} ${a.last_name}`.trim()||"Unnamed Admin"}</b></td><td>{a.email}</td><td>{a.must_change_password?<span className="pill amberpill">Password change required</span>:<span className="pill greenpill">Completed</span>}</td><td><span className={`pill ${a.is_active?"greenpill":"amberpill"}`}>{a.is_active?"Active":"Inactive"}</span></td><td><div className="admin-actions"><button className="mini-btn" onClick={()=>setEditing(a)}>Edit</button><button className="mini-btn" disabled={busy===a.user_id} onClick={()=>toggleAdmin(a)}>{a.is_active?"Deactivate":"Activate"}</button><button className="mini-btn danger-btn" disabled={busy===a.user_id} onClick={()=>removeAdmin(a)}>Remove</button></div></td></tr>):<tr><td colSpan={5}>No School Admin has been assigned yet.</td></tr>}
    </tbody></table></div>

    {editing&&<div className="modal-backdrop" onClick={()=>setEditing(null)}><div className="modal-card" onClick={e=>e.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">EDIT ADMINISTRATOR</span><h2>Update School Admin</h2></div><button className="modal-close" onClick={()=>setEditing(null)}>×</button></div><form onSubmit={updateAdmin} className="tenant-form"><label><span>First Name</span><input name="first_name" defaultValue={editing.first_name} required/></label><label><span>Last Name</span><input name="last_name" defaultValue={editing.last_name} required/></label><label className="wide"><span>Email Address</span><input name="email" type="email" defaultValue={editing.email} required/></label><label className="wide"><span>Reset Temporary Password <small>(optional)</small></span><input name="temporary_password" type="password" minLength={8} placeholder="Leave blank to keep current password"/><small>If set, the admin will be forced to create a new password at their next login.</small></label><div className="form-actions wide"><button type="button" className="secondary-btn" onClick={()=>setEditing(null)}>Cancel</button><button className="primary" disabled={busy===editing.user_id}>{busy===editing.user_id?"Saving...":"Save Changes"}</button></div></form></div></div>}
  </section>;
}
